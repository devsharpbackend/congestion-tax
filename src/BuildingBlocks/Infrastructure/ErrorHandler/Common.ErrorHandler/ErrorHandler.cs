...
class ErrorHandler : IErrorHandler
{
    ...
    {
        string ErrorMessage = ex.Message ?? "Error - Please contact the manager ";
        ...
        if (ex.GetType() == typeof(DbUpdateException))
        {
            ...
            {
                ErrorMessage = GetSqlServerError(ex.InnerException as Microsoft.Data.SqlClient.SqlException);
            }
            if (ex.InnerException.GetType() == typeof(SqlException))
            {
                ErrorMessage = GetSqlServerError(ex.InnerException as SqlException);
            }
        }
        jsonErrorResponse.Messages = ErrorMessage;

        return jsonErrorResponse;
    }

    private string GetSqlServerError(Microsoft.Data.SqlClient.SqlException exsql)
    {
        ...
        if (exsql.Number == 547)
        {
            ErrorMessage = " Because the information is dependent on other parts, it cannot be changed -- Forigin Key Constarint ";
